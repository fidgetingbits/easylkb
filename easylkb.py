...
class Kbuilder:
    def __init__(self, KConfig=None, KPath=None, KVersion="", KHostname="localhost"):
        ...
        self.runkScript += "qemu-system-x86_64"
        self.runkScript += " -m 2G"
        self.runkScript += " -smp 2"
    ...
    def KDownload(self):
        if self.KVersion:  # This means we're downloading a mainline kernel
            ...
            self.logb("warn", "You must set self.KVersion before using KDownload().")

    def KConfigure(self):
        cmdret = self.run(["make", "defconfig"], rcwd=self.KPath)
        if cmdret != 0:
            self.logb("fail", "Error running make defconfig.")
            sys.exit(1)
        cmdret = self.run(["make", "kvm_guest.config"], rcwd=self.KPath)
        if cmdret != 0:
            self.logb("fail", "Error running make kvm_guest.config.")
            sys.exit(1)
        ...
        if cmdret != 0:
            self.logb("fail", "Error running make olddefconfig.")
            sys.exit(1)
    ...
    def KCompile(self):
        ...
        if cmdret != 0:
            self.logb("fail", "Error compiling kernel.")
            sys.exit(1)
    ...
    def DebImageRun(self):
        ...
        if bash is None:
            self.logb("fail", "bash not found in PATH.")
            sys.exit(1)
        cmdret = self.run([bash, self.runkPath], rcwd=self.ImgPath)
        if cmdret != 0:
            self.logb("fail", "Error running runk.sh.")
            sys.exit(1)
